...
describe('LoginComponent', () => {
  ...
  it('should call AuthService.login with the entered credentials and rememberMe flag', () => {
    ...
    // assert & verify
    expect(mockAuthService.login).toHaveBeenCalledWith({username: 'jdoe', password: 'hunter2'}, true);
  });
  ...
  it('should default rememberMe to false when not checked', () => {
    ...
    // assert & verify
    expect(mockAuthService.login).toHaveBeenCalledWith({username: 'jdoe', password: 'hunter2'}, false);
  });
  ...
  it('should submit real credentials entered through the DOM', () => {
    ...
    fixture.debugElement.query(By.css('#password input')).nativeElement.dispatchEvent(new Event('input'));
    fixture.detectChanges();
